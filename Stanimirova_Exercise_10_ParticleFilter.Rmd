...
In today's exercise we're going to explore the use of the particle filter to constrain a simple ecosystem model for the Metolius Ameriflux sites in Oregon. Specifically, we'll be looking at the Metolius Intermediate Pine site (US-ME2) http://ameriflux.ornl.gov/fullsiteinfo.php?sid=88 and assimilating a derived MODIS data product, LAI. In the code below we will perform three analyses:
...
SSEM.orig <- function(X,params,inputs,timestep=1800){ 
  ...
  #SLA is specific leaf area - ratio of leaf area to dry mass 
  #LAI - characterizes plant canopies, one sided green leaf area per unit ground surface area 
  # X - biomass (in cabron units)
  if(inputs$PAR>1e-20){
      GPP = params$alpha*(1-exp(-0.5*LAI))*inputs$PAR
      #alpha is the light use efficiency
  } else {
      GPP = rep(0,ne)
  }
  ...
  # what is falloc??
  # why do we do the pmin and pmax ??
...
ne = 2000 ## production run should be 200 - 5000, depending on what your computer can handle
...
nt = 365*48 #fot nt - 17,000 30 min intervals 
...
bin = 86400/timestep  # 86400 seconds in a day 
...
# why are we doing this?? i dont understand ??
...
**Rerun without parameter uncertainty** 

```{r}

# #params$falloc = rdirichlet(ne,Neff%*%alloc)  ## Ra, wood, leaf
# k=as.matrix(rep(alloc,ne))
# k1=matrix(k,ncol=ne)
# params$falloc=t(k1)

params$falloc = t(matrix(alloc,nrow=3,ncol=ne))

#params$SLA = rnorm(ne,mean(SLA),sd(SLA))
params$SLA = rep(mean(SLA),ne)

params$alpha = rlnorm(ne,log(0.02),0.05) ## light use efficiency
params$alpha = rep(0.02, ne)
  
#params$Q10 = rnorm(ne,2.1,0.1)  ## soil respiration Q10
params$Q10 = rep(2.1, ne)

#params$Rbasal = rlnorm(ne,log(0.2),1)/(params$Q10^2.5) #umol/m2/sec per Mg/ha of SOM
params$Rbasal = rep(0.2, ne)/(params$Q10^2.5)
...
lit = (mean(litter)/sqrt(2))/
  (mean(Bleaf)/sqrt(2))
lit.mu = mean(lit)*timestep/86400/365 # turnover per year -> turnover per
params$litter = rep(lit.mu, ne)
...
CWD.mu = 1/142*timestep/86400/365
params$CWD = rep(CWD.mu, ne)

## Process error
params$tau.leaf = 1/sqrt(rep(10,ne)) ## prior process error in leaf biomass
params$tau.stem = 1/sqrt(rep(10,ne)) ## prior process error in stem biomass
params$tau.soil = 1/sqrt(rep(10,ne)) ## prior process error in soil carbon
...
Initial ensemble forecast for the system
...
Finally, let's implement the resampling particle filter. 
