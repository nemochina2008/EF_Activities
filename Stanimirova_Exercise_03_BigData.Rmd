...
```{r}
## read in fia csv from the web
fia = read.csv("http://apps.fs.fed.us/fiadb-downloads/RI_SEEDLING.CSV")
## sum number of trees per species 
y <- tapply(fia$TREECOUNT, as.factor(fia$SPCD), sum, na.rm=TRUE)
y_sort <- sort(y, decreasing=TRUE) # sort species abundance in decreasing order 
y_sort_log <- log(y_sort) # take the lof of sorted values

## plot the log(abundance) of tree seedinling versus rank
plot(y_sort_log, 
     cex=1.5,    		# increase the symbol size
     cex.axis=1.3,			# increase the font size on the axis 
     xlab="Rank",		# label the x axis
     ylab="Log Abundance",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
     main="Tree Seedling Counts Rhode Island", # title
     cex.main=1.3,			# increase title font size
     type='l')
...
```{r}
flux_html = getURL("http://fluxnet.ornl.gov/site_status")  ## grab raw html
flux_table = readHTMLTable(flux_html)    ## read in table from this webpage
flux_df <- as.data.frame(flux_table)     ## save it as a data frame 
## extract country code and add it as a column in the table 
flux_df$CntryCode <- substring(flux_df$status_table.FLUXNET.ID, 1,2)  

flux_sort <- flux_df[order(flux_df$CntryCode),] ## sort data by country code 
## create sorted table of flux towers in each country 
flux_table <- as.data.frame(table(flux_sort$CntryCode))  
colnames(flux_table) <- c("CntryCode", "NumFluxTowers") ## name columns 

flux_table$CntryCode <- sub("Au","AU",flux_table$CntryCode,ignore.case=FALSE)  ## substitute ALL: VT for RI
aggregate(flux_table$NumFluxTowers, by=list(name=flux_table$CntryCode), sum)
...
```{r}
z = grep("^#",myCode)    ## returns the line numbers that include the string 'RI'
z
myCode[z]
...
```{r}
system("wget http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/catalog.html?dataset=1220/mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
daac = nc_open("http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
print(daac)    ## metadata

lon = ncvar_get(daac,"lon")    ## longitude data
lat = ncvar_get(daac,"lat")    ## latitude data
time = ncvar_get(daac,"time")    ## time data
time_year <- as.Date(time, origin="1700-01-01")

lat_b = which(lat == 42.25)  ## approximately Boston's location 
lon_b=which(lon==-71.25)     ## closest lon to Boston that I found in the data set 
t = length(time)

tair = ncvar_get(daac,"tair",c(lon_b,lat_b,1), c(1,1,t))

plot(time_year,tair,
     cex=1.5,      	# increase the symbol size
     cex.axis=1.3,			# increase the font size on the axis 
     xlab="Time",		# label the x axis
     ylab="Air Temperature (K)",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
     main="Boston Air Temperature in 2004", # title
     cex.main=1.3,			# increase title font size
     type='l')
...
```
  ## arguments are the library name and the username 
...
When NEE is negative, EVI is high. This makes sense because when greenness is highest during the summer in the Northern Hemisphere when photosynthesis exceeds respiration as plants accumulate biomass. During the winter time when photosynthesis is low, respiration dominates, and NEE is positive and EVI is low. 



```{r}
## print fluxes at 3 different heights
doy_1 <- as.Date(doy, origin="2012-01-01")
...
plot(time,EVI,
     ...
     cex.axis=1.3,  		# increase the font size on the axis 
     xlab="Time",		# label the x axis
     ylab="EVI",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
     main="WLEF Flux Cite MODIS EVI Timeseries", # title
     cex.main=1.3,			# increase title font size
     type='l')
par(new=TRUE)

plot(doy_1, filter(NEE[1,],rep(1/24,24)), col="red",
     xaxt="n",yaxt="n",
     xlab="",ylab="",
     type='l')
axis(4, cex.axis=1.3, cex=1.5)
mtext("NEE",side=4,line=3,      
     cex.lab=1.3)
legend("topleft",col=c("black","red"),lty=1,legend=c("EVI","NEE"))
...
1. Define settings, load required libraries 

2. Access data and web scraping 
    - use the US Forest Service's Forest Inventory and Analysis (FIA) data set 
    - Data is available at http://apps.fs.fed.us/fiadb-downloads/ 
    - if data formated in HTML then scrape webpage using the RCurl library to grab HTML/XML content from web
        * grab raw html using getURL() 
   
3. Keep files up to date
    - grab data on the status of files from FIA in order to check when they have been updated 
    - plot a histogram of the dates when the data was updated 
    - tabulate updated files and grab only those that have been updated in the last week 

4. Read and organize data
    - then use XMl library to parse markup into R data objects
    - read in updated tables using readHTMLTable()

5. Write out and save data 
...
MAILTO=rkstan@bu.edu
0 0 * * 0 /Users/stanimirova/Documents/EF_Activities/NOMADS/get_sref.sh
```
