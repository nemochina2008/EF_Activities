...
## call jags model to establish connection with jags, compile the model and run through
j.model   <- jags.model (file = textConnection(NormalMeanN),
                             data = data,
                             inits = inits,
                             n.chains = 3)

## once the model is compiled then coda samples is called to sample from the posterior
jags.out   <- coda.samples (model = j.model,
                            variable.names = c("mu"),
                                n.iter = 1000)
...
## additional samples
jags.out2 <- coda.samples(j.model,variable.names = c("mu"),3000)
plot(jags.out2)

## compares the among chain variance to the within change variance 
gelman.diag(jags.out)
gelman.diag(jags.out2)

## determine when convergence happened is done both visually and by plotting the GBR statistic versus sample
GBR <- gelman.plot(jags.out)
...
## determine number of independent samples (effective sample size)
effectiveSize(jags.burn)

## plot the evolution of sample quantiles as a funcion of number of iterations
cumuplot(jags.burn,probs=c(0.025,0.25,0.5,0.75,0.975))

## calculate summary of output 
summary(jags.burn)
...
The mean diameter is approximately 16, with standard deviation of ~ 0.3, and time-series SE of ~0.003. Both SD and SE are low indicating low uncertainty around the parameter mu and high numerical preceision of the results, respectively.  

* Graph of parameter density and plot of MCMC “history”

I know the model has converged because the chains have overlapping trajectories and bounce around at random, showing some longer term trends. The trajectories of the chains are consistent over time and their distribution looks appropriately normal.
...
I made one update including many more iterations (10,000 as opposed to 1,000), which improved the appearance of the trace and density plots. With fewer samples there was a trend in the trace plot and the normal distribution wasn't as tight as it was with more samples. However, the GBR for both is approximately 1 showing that the model has converged. The effective sample size is approximately 3000 so the final analysis was performed with 3 chains with 1000 iterations. 
I also tried working with 4 chains instead of 3 using the following initial values. It actually slightly increased the GBR value but didn't not change the summary statistics, which showed similar mean and timeseries standard errors. 
...
The burnin happened within the first few iterations because the model converged almost immediately with identical chains corresponding to GBR values of 1. Convergence has happened when the output from all chains is indistinguishable 
The effective sample size approximates the number of independent samples, which in this case is approximately 3000 samples for estmating the mean 
...
model {
  mu ~ dnorm(mu0,T) # prior on the mean 
  S ~ dunif(0,100) # prior on the variance
  for(i in 1:N){
    X[i] ~ dnorm(mu,S) # data model
  }
}
...
## set initial values for mu and S 
inits <- list()
inits[[1]] <- list(mu = 15, S=1/20)
inits[[2]] <- list(mu = 20, S=1/25)
inits[[3]] <- list(mu = 25, S=1/30)

## call jags model to establish connection with jags, compile the model and run through
j.model   <- jags.model (file = textConnection(NormalMeanNS),
                             data = data,
                             inits = inits,
                             n.chains = 3)
...
## determine the first iteration after convergence
burnin <- GBR$last.iter[max(tail(which(GBR$shrink[,1,2] > 1.1),1))+1]
## check for no burn-in case
if(length(burnin) == 0) burnin = 1
## remove burn-in
jags.burn <- window(jags.out,start=burnin)
...
I selected a uniform prior for the precision because my precision cannot be negative and with a uniform function I can restrict the range of values that S can take from 0 to 100. Additionally, I am not sure if any S value is more likely than another and in an uniform distribution all values between 0 and 100 have equal probability. Unifrom distribution plays nicely with normal distribution, which was another consideration. Precision is a continuous varibale so the uniform distribution is a good fit in that sense as well. 

* Table of summary statistics for the posterior mean and standard deviation
Summary statistics show precision of ~0.03 as expected and mean of ~16 with low time-series SE. Both SD and SE are low indicating low uncertainty around the parameter mu and high numerical preceision of the results, respectively.

* Graph of parameter densities and plot of MCMC “history”
...
- I made one update including many more iterations (3,000 as opposed to 1,000), which improved the appearance of the trace and density plots. With fewer samples there was a trend in the trace plot and the normal distribution wasn't as tight as it was with more samples. The effective sample size is approximately 5,000 for S and 8,500 for mu so the final analysis was performed with 3 chains and 3,000 iterations which get us to an effective sample size. 

- The burnin happened within the first few iterations because the model converged almost immediately with identical chains corresponding to GBR values of 1. Convergence has happened when the output from all chains is indistinguishable 

- The effective sample size approximates the number of independent samples, which in this case is approximately 5,000 for S and 8,500 for mu

* Also describe any changes in the distribution of the mean (shape, location, tails) compared to Task 1.
I set the burnin value to be determoned based on S since it seemed to have GBR values higher than 1.1 more often than mu. However, sometimes I get an error message. Ideally, I want to look at both columns simulataneously and set my burnin value according to when both mu and S are below 1.1. This would be done by using which.mac command. 

Summary statistics between Task 1 and Task 2 seem to be very comparable (mu, s.d., SE and qauntiles) with the major exception that Task 2 includes S in addition to mu
A visual comparsion shows similar shapes and tails. However, the density of mu in Task 2 is slightly less thight with a slight bulge at the top right. 
Location is very similar
