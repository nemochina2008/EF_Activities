...
I know the model has converged because the chains have overlapping trajectories and bounce around at random, showing longer term trends. The trajectories of the chains are consistent over time and that their distribution looks appropriately normal.
...
I made one update including many more iterations (10,000 as opposed to 1,000), which improved the appearance of the trace and density plots. With fewer samples there was a trend in the trace plot and the normal distribution wasn't as tight as it is with more samples. However, the GBR for both is 1 showing that the model has converged. So I will perform the rest of the analysis on the smaller sample.  
...
## determine the first iteration after convergence
burnin <- GBR$last.iter[max(tail(which(GBR$shrink[,,2] > 1.1),1))+1]
## check for no burn-in case
if(length(burnin) == 0) burnin = 1
## remove burn-in
jags.burn <- window(jags.out,start=burnin)
