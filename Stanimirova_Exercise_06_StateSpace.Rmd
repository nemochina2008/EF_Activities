...
```{r}
plot(time,ci.monthly[2,],type='n',ylim=range(y,na.rm=TRUE),ylab="Flu Index",log='y',xlim=time[time.rng])
## adjust x-axis label to be monthly if zoomed
if(diff(time.rng) < 100){ 
  axis.Date(1, at=seq(time[time.rng[1]],time[time.rng[2]],by='month'), format = "%Y-%m")
}
...
```{r}
plot(y, ci[2,], xlab="Observed", ylab="Predicted")
abline(0,1, col="blue", lwd=1.5, lty=2)

plot(y, ci.monthly[2,], xlab="Observed", ylab="Predicted")
abline(0,1, col="blue", lwd=1.5, lty=2)
...
plot(time[s],ci.rm40[2,s],type='n',ylim=range(y.rm40[s],na.rm=TRUE),ylab="Flu Index",log='y',xlim=time[time.rng.s], xaxt='n')
## adjust x-axis label to be monthly if zoomed
